...
if [ -z "$SLURM_JOB_ID" ]; then
  ...
  if [ -z "$PC2SYSNAME" ]; then
    ...
    export HF_HOME="$WORK_HOME/hf_cache"
...
run="run=train run.epochs=10 model=tinystack run.export=True debug=True +trace=True"
...
for model in ${models[@]}
do
    echo qtransform $run model=$model $data $quant
    qtransform $run model=$model $data $quant
done
