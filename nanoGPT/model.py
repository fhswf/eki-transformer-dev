...
#from brevitas.nn import functional as qF
...
class CausalSelfAttention_with_MHA(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.quantize:
            ...
            self.c_attn = qnn.QuantLinear(config.n_embd, 3 * config.n_embd, bias=config.bias, weight_bit_width=8, bias_bit_width=32)
            # output projection
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.quantize:
            # key, query, value projections for all heads, but in a batch
            self.c_attn = qnn.QuantLinear(config.n_embd, 3 * config.n_embd, bias=config.bias, weight_bit_width=8, bias_bit_width=8)
            # output projection
            self.c_proj = qnn.QuantLinear(config.n_embd, config.n_embd, bias=config.bias, weight_bit_width=8, bias_bit_width=8)
        else:
            # key, query, value projections for all heads, but in a batch
            self.c_attn = nn.Linear(config.n_embd, 3 * config.n_embd, bias=config.bias)
            # output projection
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn = CausalSelfAttention_with_MHA(config)
