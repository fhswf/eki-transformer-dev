import os
from qtransform.dataset import TokenizedDatasetGenerator, DatasetSplits, DatasetSplitType, MODEL_INPUT_NAME, \
    MODEL_LABEL_NAME, MODEL_MASK_NAME
...
class HuggingfaceTokenizedDatasetGenerator(TokenizedDatasetGenerator):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.DUMP_FILE_PATH = join(self.DATASET_FILE_PATH, "tokenized",
                                   "")  # tokenized datasets are not grouped to block_size yet
    ...
    def tokenize_data(self, untokenized_data: DatasetDict) -> None:
        ...
        text_column_name = untokenized_data[list(untokenized_data.keys())[0]].column_names[
            0]  # check in what column name the data is stored
        ...
        dump_file_names = {split.name: self.DUMP_FILE_PATH + self.CACHE_FILENAME_PREFIXES[split] + "tokenized.arrow" for
                           split in DatasetSplitType}
        ...
        log.debug(
            f'First sample: {len(lm_datasets[DatasetSplitType.EVAL.name]["input_ids"][0])}')  # make sure it is the length of block_size
    ...
    def get_untokenized_data(self, splits: List[DatasetSplitType]) -> DatasetDict:
        ...
        if fetched_splits is None:
            log.error(
                f'Could not fetch splits from Huggingface API endpoint, either because the specified config has errors or because the repository is private.' \
        ...
        for split in splits:
            ...
            try:
                ...
                if not split_cfg.exists:
                    log.info(
                        f'Creating split: "{split_cfg.split}" from "{split_cfg.mapping}" with size: {split_cfg.size}.')
                    ...
                    if check_split_loaded is not None and dataset_splits.get(check_split_loaded.name, None) is not None:
                        ...
                        reduced_dataset = dataset_splits.get(check_split_loaded.name, None).train_test_split(
                            split_cfg.size)
                ...
                log.error(
                    f'Split "{split.name}" does not exist with "exists: {split_cfg.exists}", "mapping: {split_cfg.mapping}". Maybe check mapping?')
            ...
            dataset_split = dataset_split.remove_columns(["code", "combined"])

            # combine columns into one, separated by special tokens
            combined_column_name = "combined"
            dataset_split = dataset_split.map(lambda batch: {
                combined_column_name: [
                    f"<code> {faulty_code} </code> <error_message> {error_message} </error_message> <description> {description} </description>"
                    for faulty_code, error_message, description in zip(
                        batch["faulty_code"], batch["error_message"], batch["description"]
                    )
                ]
            }, batched=True)

            # remove columns that are not needed
            dataset_split = dataset_split.remove_columns(["faulty_code", "error_message", "description"])
            dataset_splits[split.name] = dataset_split
    ...
    def get_collator(self) -> Callable:
        ...
        return DataCollatorWithPadding(tokenizer=tokenizer, padding='max_length',
                                       max_length=self.cfg.tokenized.args.block_size)
...
def pad(self,
    ...
    assert len(
        labels) == batch_size, f'Batch size for input_ids ({batch_size}) and labels ({len(input_ids)}) are different'
